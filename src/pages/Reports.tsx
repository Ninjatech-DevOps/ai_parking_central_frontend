...
import {
  reportsApi, areasApi, locationsApi, downloadFile,
} from "@/services/api";
...
import {
  BarChart3, Car, Bike, Clock, Download, FileDown, FileSpreadsheet,
...
export default function Reports() {
  ...
  // ─── Generate the combined report (parking + ANPR) from the live API ───
  const generate = useCallback(async () => {
    ...
    const [rep, anprSess, anprSum, anprLoc] = await Promise.allSettled([
      reportsApi.summary(params),
      reportsApi.anprSessions(params),
      reportsApi.anprSummary(params),
      reportsApi.anprLocations(params),
    ]);
    setData(rep.status === "fulfilled" ? rep.value.data : null);
    setAnprSessions(anprSess.status === "fulfilled" ? (anprSess.value.data.items || []) : []);
    setAnprSummary(anprSum.status === "fulfilled" ? anprSum.value.data : null);
    setAnprLocations(anprLoc.status === "fulfilled" ? (anprLoc.value.data.locations || []) : []);
    setLoading(false);
  }, [buildParams]);
  ...
  const analyzeOccupancy = useCallback(async () => {
    ...
    try {
      const { data } = await reportsApi.occupancyAnalysis(
        buildParams({ threshold: String(occThreshold), ...(occSlotType ? { slot_type: occSlotType } : {}) })
      );
      setOccData(data);
    } catch { setOccData(null); }
    setOccLoading(false);
  }, [range, occThreshold, occSlotType, buildParams]);
  ...
  // ─── Exports (Excel & PDF generated by backend; downloaded directly) ───
  ...
  // Unified export — one backend-generated file with ALL tabs (Excel = multi-sheet, PDF = full report).
  async function handleExport(format: "excel" | "pdf") {
    const ts = new Date().toISOString().slice(0, 10);
    const p = buildParams({ threshold: String(occThreshold), ...(occSlotType ? { slot_type: occSlotType } : {}) });
    if (format === "pdf") {
      doDownload(reportsApi.exportPdfUrl(p), `reports_${ts}.pdf`);
    } else {
      doDownload(reportsApi.exportExcelUrl(p), `reports_${ts}.xlsx`);
    }
  }
...
function CoveragePanel({ rangeLabel }: { rangeLabel: string }) {
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 to-white rounded-2xl card-shadow p-5 print-break-avoid">
      ...
      <p className="text-[11px] text-slate-400 mb-4">Every metric below is included across the AI Parking and ANPR tabs and in Excel / PDF exports.</p>
